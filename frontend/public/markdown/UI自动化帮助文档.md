...
**CLASS_NAME 定位：**
- 定位表达式：`btn` 或 `btn-primary` 或 `btn-large`
- 说明：直接使用 class 属性中的任意一个值，不需要点号
- 限制：如果元素有多个 class 值（如 `class="btn btn-primary"`），CLASS_NAME 只能匹配其中一个

**CSS_SELECTOR 定位：**
- 定位表达式：`.btn` 或 `.btn-primary` 或 `.btn-large`
- 组合定位：`.btn.btn-primary`（同时匹配两个 class）
- 标签+类组合：`div.btn-primary` 或 `button.btn-large`
- 说明：使用 CSS 选择器语法，需要点号开头，可以灵活组合

**选择建议：**
- 如果元素只有一个 class 值，且该 class 值在页面中唯一，可以使用 **CLASS_NAME**（表达式更简洁）
- 如果需要组合多个 class 或标签，或者需要更灵活的定位，使用 **CSS_SELECTOR**（功能更强大）
